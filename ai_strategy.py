...
def get_daily_market_report(macro, us_news, kr_news, research_reports, extra):
    ...
    [ 최종 행동 지침 ]
    - (아래 4개 중 1개 선택 기입: [적극매수] / [보수적 매수] / [현금 관망] / [비중 축소])
    - 대상 섹터: (명시)
    """
    return generate_text(prompt)
...
def get_deep_market_report(macro, kr_news, research_reports):
    prompt = f"""
    10:00 기준 한국 시장 자금 흐름을 분석합니다. 스마트폰 가독성을 위해 간결하게 작성하세요. (이모지 금지)
    ...
    [ 최종 행동 지침 ]
    - (선택: [적극매수] / [보수적 매수] / [현금 관망] / [비중 축소])
    - 대상 섹터: (명시)
    """
    return generate_text(prompt)

def get_weekly_portfolio_report(portfolio, news_dict):
    prompt = f"보유 종목({portfolio})의 투자 이유 유효성을 뉴스({news_dict}) 기반으로 냉정히 점검하세요. 스마트폰 가독성을 위해 짧은 개조식으로 작성. 이모지 금지."
    return generate_text(prompt)

def get_monthly_portfolio_report(portfolio, news_dict):
    prompt = f"포트폴리오({portfolio}) 및 뉴스({news_dict}) 분석. 성장주/가치주 트렌드 및 리밸런싱 가이드를 모바일 가독성에 맞춰 짧게 작성. 이모지 금지."
    return generate_text(prompt)

def get_quarterly_portfolio_report(portfolio, news_dict):
    prompt = f"포트폴리오({portfolio}) 기업 실적 훼손 여부 점검. 훼손 시 매도 권고. 모바일에서 읽기 편하게 핵심만 작성. 이모지 금지."
    return generate_text(prompt)
...
def check_fundamental_damage(ticker, stock_name, chart_30d, macro, valuation, theme_context):
    prompt = f"""
    투자 훼손 여부 진단. 모바일 가독성을 위해 짧게 작성. (이모지 금지)
...
def get_quick_rating(ticker, name, news_text, strategy_type, val):
    prompt = f"""
    스마트폰 가독성을 위해 아주 짧고 명확하게 5단계 등급과 이유를 판정하세요. (이모지 금지)
...
def get_dividend_risk_check(ticker, name, news_text, val):
    prompt = f"""
    배당주 컷 위험을 검증합니다. 스마트폰 가독성을 위해 아주 짧고 명확하게 출력하세요. (이모지 금지)
