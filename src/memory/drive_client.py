...
def get_relative_file_modified_time(rel_path):
    """임의 상대 경로 파일의 Google Drive modifiedTime(ISO 문자열) 반환.

    파일이 없거나 조회 실패 시 ``None``.
    """
    try:
        _check_pause_guard()
        svc, _root_id, parent_id, fname = _folder_id_for_relative(rel_path)
        q = (
...
def get_app_file_modified_time(filename):
    ...
    """
    rel = f"{APP_DATA_PREFIX}/{filename}"
    return get_relative_file_modified_time(rel)
