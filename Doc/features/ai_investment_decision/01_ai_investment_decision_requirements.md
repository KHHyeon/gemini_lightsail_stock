...
## 구현율
- v1.3.1 (2026-05-25): 자동매도 정책 — 수동등록은 추적익절만 — 구현 완료
- v1.3 (2026-05-25): 5축 GARP 채점 + 분석보류 라벨 + AI ±2 확장 — 구현 완료
...
R9. 자동매도 정책 — 등록 출처에 따른 차등 적용 (v1.3.1 신규, 2026-05-25)
  - `daily_fundamental_stop_loss` (14:30 안전진단) 의 3중 방어막은 **등록 출처에 따라 차등 적용**한다.
  - **자동 발굴 / AI매수(`!ai매수`) 종목**: 3중 방어막 전체 적용.
    1. 원금 대비 -10% 이탈 → 기계적 손절.
    2. 최고점 대비 -10% 이탈 → 추적 익절/손절.
    3. AI 펀더멘털 훼손 (`check_fundamental_damage` 가 `[펀더멘털훼손]` 반환) → 자동 매도.
  - **수동등록(`!수동등록`) 종목**: 자동 매도는 **추적 익절(2) 만** 적용.
    - 원금 손절(1) 과 펀더멘털 훼손 매도(3) 는 스킵한다.
    - 사용자가 직접 매수 의사를 명시한 종목은 시장 가격이 평균가에서 벗어나거나 AI 가 펀더멘털 악화로 판단해도 시스템이 임의로 매도하지 않는다 (사용자 의사 존중).
    - 단, 한 번이라도 평균가보다 상승하여 최고점이 갱신된 후 -10% 이상 하락(추적 익절 조건)할 경우에는 이익 보존을 위해 자동 매도를 수행한다.
  - 식별 키: `portfolio[ticker].reason` 문자열에 "수동등록" 포함 여부.
  - AI 펀더멘털 훼손 점검은 수동등록 종목에 대해 **호출 자체를 스킵**하여 토큰을 절약한다.
