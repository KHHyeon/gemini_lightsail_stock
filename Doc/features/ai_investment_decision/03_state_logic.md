...
버전별 구현율: v1.0 (100% 설계, 100% 구현 — 본 PR 기준)
...
- 라벨 수/순서 변경 시 본 표와 `02_api_spec.md` 를 동시 갱신해야 한다.
...
```
[테마: {target_theme} | 총점: {score}]
{narrative}
```

- `target_theme` 결정 우선순위:
  1. `!발굴` 경로: 네이버 공식 테마 매핑 결과 그대로.
  2. `!ai매수` 경로: `score_single_ticker` 가 산출한 `target_theme`
     (예: `"가치성장 대장주"`, `"우량 금융주"`). 산출 실패 시 `"AI매수(단일종목)"`.
...
```
[한줄요약] [의견: {opinion_final}] | [점수: {score} ({opinion_code} → 보정 {delta:+d})] | [근거] {llm_rationale} | [상승조건] {llm_upside} | [손절조건] {llm_downside}
```
